...
def debug_tournaments_dup(user_id: str = "335f7c35-320e-4671-a90e-e57062792e5a"):
    try:
        ...
        import polars as pl
        df = cache["df_hands"]
        t = df.filter(pl.col("game_type") == "Tournament").select("source_file").unique().to_dict(as_series=False)
        r = df.filter(pl.col("game_type") == "Regular Cash").select("source_file").unique().to_dict(as_series=False)
        return {"t": t, "r": r}
