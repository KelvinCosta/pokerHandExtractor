...
class HandLoader:
    ...
    def transform_hand(self, hand: HandContext) -> dict:
        ...
        return {
            ...
            "starting_stacks": str(hand.starting_stacks) if hasattr(hand, "starting_stacks") else "{}",
            "player_seats": str(hand.player_seats) if hasattr(hand, "player_seats") else "{}",
            "button_seat": hand.button_seat if hasattr(hand, "button_seat") else 0,
    ...
    def save_dict_batch(self, dict_batch: list[dict], run_id: int, batch_index: int) -> int:
        if not dict_batch:
            return 0
    ...
    def process_and_save(self, hands_iterator: Iterator[HandContext], batch_size: int = 10000) -> int:
        ...
        while True:
            ...
            dict_batch = [self.transform_hand(h) for h in batch]
            total_processed += self.save_dict_batch(dict_batch, run_id, batch_index)
            batch_index += 1
