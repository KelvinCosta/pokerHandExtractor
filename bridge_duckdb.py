...
def extract_player_metrics(player_id: str, days_limit: int, parquet_path: str = "hands_mock.parquet") -> PlayerStats:
    ...
    query = f"""
        SELECT 
            ...
            CAST(COALESCE(SUM(profit), 0) AS DOUBLE) as financial_profit,
    ...
    try:
        # Na produção, usaríamos duckdb.query(query).fetchone()
        # Como o parquet_path é fictício, vamos simular o retorno da query
        # result = duckdb.query(query).fetchone()
        
        # Mock do resultado do DuckDB para permitir testes isolados sem o Parquet real
        result = (player_id, 1250.75, 2.8, 26.5, 4, 0)
